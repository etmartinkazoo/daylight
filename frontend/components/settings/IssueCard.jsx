      ...
      {expanded && (
        <div className="border-t px-4 py-4 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <>
                {issue.source_location && (
                  <div className="flex flex-col gap-0.5">
                    ...
                    <code className="text-sm font-mono bg-muted/50 px-1.5 py-0.5 rounded">{issue.source_location}</code>
                  </div>
                )}
                {issue.controller_action && (
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm text-muted-foreground font-medium">Controller</span>
                    <code className="text-sm font-mono bg-muted/50 px-1.5 py-0.5 rounded">{issue.controller_action}</code>
                  </div>
                )}
              ...
              <>
                {issue.file_path && (
                  <div className="flex flex-col gap-0.5 col-span-2">
                    <span className="text-sm text-muted-foreground font-medium">File</span>
                    <code className="text-sm font-mono bg-muted/50 px-1.5 py-0.5 rounded">{issue.file_path}{issue.line_number ? `:${issue.line_number}` : ""}</code>
                  </div>
                )}
                {issue.warning_type && (
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm text-muted-foreground font-medium">Warning Type</span>
                    <span className="text-sm">{issue.warning_type}</span>
                  </div>
                )}
                {issue.check_name && (
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm text-muted-foreground font-medium">Check</span>
            ...
            {issue.detected_at && (
              <div className="flex flex-col gap-0.5">
                <span className="text-sm text-muted-foreground font-medium">Detected</span>
          ...
          {variant === "performance" && issue.sql_pattern && (
            <div className="flex flex-col gap-1.5">
              <span className="text-sm text-muted-foreground font-medium">SQL Pattern</span>
              <pre className="text-sm font-mono bg-muted/50 rounded-md p-3 overflow-x-auto">{issue.sql_pattern}</pre>
            </div>
          )}

          {variant === "security" && issue.code_snippet && (
            <div className="flex flex-col gap-1.5">
              <span className="text-sm text-muted-foreground font-medium">Vulnerable Code</span>
              <pre className="text-sm font-mono bg-muted/50 rounded-md p-3 overflow-x-auto">{issue.code_snippet}</pre>
            </div>
          )}
